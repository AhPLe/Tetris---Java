...
//	public final PieceType type;
	...
	public Piece(int x, int y, PieceType type, Handler handler, PieceBoard pieceBoard) {
	...
	public boolean moveOne() {
		...
		if (!(pieceBoard.isBlocked(x, y + BRICKSIZE, this.shape))){
			y += BRICKSIZE;
			arrY += 1;
			return true;
		}
	...
	public void moveRight() {
		if (!(pieceBoard.isBlocked(x + BRICKSIZE, y, this.shape))){
			x += BRICKSIZE;
			arrX += 1;
		}
	}
	...
	public void moveDown() {
		if (verbose)
			System.out.println("Piece go down");
		arrY = pieceBoard.lowestPoint(this);
		y = arrY *BRICKSIZE;
	...
	public void rotateClockwise() {
		...
		if (!(rotation_node[0] < 0)) {
			...
			if (!(pieceBoard.isBlocked(x_new, y_new, shapeNew))){
				this.shape = shapeNew;
	...
	public void rotateCounterClockwise() {
		...
		if (!(rotation_node[0] < 0)) {
			// check if the piece should rotate
			boolean[][] shapeNew = new boolean[shape[0].length][shape.length];
			//boolean[][] rotationPointNew = new boolean[rotation_point[0].length][rotation_point.length];
			int x_new, y_new;
			int y_node = rotation_node[0];
